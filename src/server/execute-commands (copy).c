...
{
    char givenUID[MAX_UID_SIZE], givenGID[MAX_GID_SIZE], givenMID[MAX_MID_SIZE];
    char messagesDirName[GROUPMSGDIR_SIZE];
    char firstMessageDir[GROUPNEWMSGDIR_SIZE], currMessageDir[GROUPNEWMSGDIR_SIZE];
    char *authorID, *message;
    char fileName[MAX_FILENAME_SIZE];
    int N = 0, currMID;
    size_t msg_size;

    sscanf(peekedMsg, "RTV %s %s %s ", givenUID, givenGID, givenMID);

    if (!(validUID(givenUID) && validGID(givenGID) && validMID(givenMID)))
    { // abort new process execution
    ...
    sprintf(messagesDirName, "GROUPS/%s/MSG", givenGID);
    sprintf(firstMessageDir, "%s/%s", messagesDirName, givenMID);

    if (!directoryExists(firstMessageDir))
    { // check if first message exists
        fprintf(stderr, "[-] There are no messages available starting with the given MID.\n");
        return "EOF";
    }

    // TO DO: Not going to work, because of zero's on the left
    currMID = atoi(givenMID);

    while (N < 20)
    {
        // Check if message with this ID exists. If not, break
        sprintf(currMessageDir, "%s/%s", messagesDirName, currMID);
        if (!directoryExists(currMessageDir))
        {
            break;
        }

        // Get author's UID
        getAuthorID(currMessageDir, authorID);

        // Get message text and text size
        getMessage(currMessageDir, message, &msg_size);

        // Get Fname, Fsize and data if exists
        char authorFileName[GROUPNEWMSGAUT_SIZE], messageFileName[GROUPNEWMSGTXT_SIZE];
        DIR *dir;
        struct dirent *ent;

        // Search for a file in the message
        sprintf(authorFileName, "%s/A U T H O R.txt", currMessageDir);
        sprintf(messageFileName, "%s/T E X T.txt", currMessageDir);
        if ((dir = opendir(currMessageDir)) != NULL)
        {
            while ((ent = readdir(dir)) != NULL)
            {
                if ((strcmp(ent->d_name, authorFileName) != 0) && (strcmp(ent->d_name, messageFileName) != 0))
                {
        ...
        { // could not open directory
            fprintf(stderr, "[-] Unable to open message directory.\n");
            exit(EXIT_FAILURE);
        }
        ...
        if (strlen(fileName) > 0)
        {
            ...
            if (fPtr == NULL)
            {
                fprintf(stderr, "[-] Unable to open file in retrieve.\n");
                exit(EXIT_FAILURE);
            }
            if (fseek(fPtr, 0, SEEK_END) == -1)
            {
                fprintf(stderr, "[-] Unable to seek file in retrieve.\n");
                exit(EXIT_FAILURE);
            }
            ...
            if (file_size == -1)
            {
                perror("[-] File tell failed in retrieve.\n");
                exit(EXIT_FAILURE);
            }
            if (fseek(fPtr, 0, SEEK_SET) == -1)
            {
                perror("[-] File seek failed in retrieve.\n");
                exit(EXIT_FAILURE);
            }
        }
        // TO DO: Read the file's data

        // TO DO: Add current message to list to send to client

        // TO DO: Increase MID

        // TO DO: Free authorID, message
    }
}