...
void processUDPMsg(char * buffer) {
    ...
    if (!strcmp(codeDS, "RRG")) { // REGISTER command
    ...
    else if (!strcmp(codeDS, "RUN")) { // UNREGISTER command
        if (!strcmp(statusDS, "OK")) {
    ...
    else if (!strcmp(codeDS, "RLO")) { // LOGIN command
        if (!strcmp(statusDS, "OK")) {
    ...
    else if (!strcmp(codeDS, "ROU")) { // LOGOUT command
        if (!strcmp(statusDS, "OK")) {
    ...
    else if (!strcmp(codeDS, "RGL")) { // GROUPS command
    ...
    else if (!strcmp(codeDS, "RGS")) { // SUBSCRIBE command
        if (!strcmp(statusDS, "OK")) {
    ...
    else if (!strcmp(codeDS, "RGU")) { // UNSUBSCRIBE command
        if (!strcmp(statusDS, "OK")) {
    ...
    else if (!strcmp(codeDS, "RGM")) { // MY_GROUPS command
        if (isNumber(statusDS)) {
            printGroups(buffer, atoi(statusDS));
        } else {
    ...
    else { // Unexpected protocol -> ABORT EXECUTION
...
void processTCPMsg(char * buffer, int * flag) {
    char codeDS[DSCODE_SIZE], statusDS[DSSTATUS_SIZE];
    sscanf(buffer, "%s %s", codeDS, statusDS);
    if (!strcmp(codeDS, "RUL")) { // ULIST command
        if (!strcmp(statusDS, "OK")) {
            buffer += 7;
    ...
    else if (!strcmp(codeDS, "RPT")) { // POST command
    ...
    else if (!strcmp(codeDS, "RRT")) { // RETRIEVE command
        if (!strcmp(statusDS, "OK")) {
            *flag = 1;
        } 
        else if (!strcmp(statusDS, "NOK")) {
...
void exchangeUDPMsg(char * message) {
    ...
    if (nUDP == -1) { // Syscall failed -> terminate gracefully 
...
void exchangeTCPPost(char * message, FILE * post, long lenFile) {
    connectTCPSocket();
    sendTCP(message);
    if (!sendFile(post, lenFile)) { exit(EXIT_FAILURE); } // sendFile already closes socket's fds and post stream upon error
    char msgRecvBuf[9];
    if(readTCP(msgRecvBuf, 9, 0) == -1) { fclose(post); exit(EXIT_FAILURE); } // readTCP already closes socket's fds upon error
...
void exchangeTCPMsg(char * message) {
    ...
    while ((n = readTCP(msgRecvBuf, TCP_READ_SIZE, 0)) > 0) { // Read all data
        if (bytesRecv + n >= lenBuf) { // Buffer isn't big enough -> must realloc
...
void exchangeTCPRet(char * message) {
    ...
    connectTCPSocket();
    sendTCP(message);
    if (readTCP(msgRecvBuf, 7, 0) == -1) exit(EXIT_FAILURE); // Read code and status
    processTCPMsg(msgRecvBuf, &flag); // flag as pointer to be set in processTCPMsg function
    if (!flag) { closeTCPSocket(); return; } // EOF or NOK
    if (readTCP(numRetMsgs, 2, 0) == -1) exit(EXIT_FAILURE);  // Read number of messages to read
    if (atoi(numRetMsgs) >= 10) { if (readTCP(backspace, 1, 0) == -1) exit(EXIT_FAILURE); } // Update stream pointer consistency (< 10 read an extra backspace)
    ...
    while(1) {
        ...
        retDel = temp;
        if (readTCP(retDel, offset, 0) == -1) exit(EXIT_FAILURE); // Read offset bytes to start reading text/file data
        free(temp); temp = NULL;
        long bytesToRead = atol(tempBuf3);
        if (strlen(tempBuf1) == 4 && isNumber(tempBuf1)) { // Text case
            if ((nRecv = readTCP(textBuf, bytesToRead + 1, 0)) == -1) exit(EXIT_FAILURE); // bytesToRead + 1 to read extra space
            ...
            if (readTCP(backspace, 1, 0) == -1); // Update stream pointer consistency
        } 
    }
    closeTCPSocket();
}
